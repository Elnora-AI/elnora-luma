...
var LumaApiError = class extends Error {
  constructor(status, statusText, body, method, path2) {
    super(`Luma API ${method} ${path2} \u2192 HTTP ${status} ${statusText}`);
    this.status = status;
...
var StripeApiError = class extends Error {
  constructor(status, statusText, body, path2) {
    super(`Stripe API GET ${path2} \u2192 HTTP ${status} ${statusText}`);
    this.status = status;
    this.statusText = statusText;
    this.body = body;
    this.path = path2;
...
// src/report.ts
var EXIT_WARNINGS = 3;
function writeOut(path2, content) {
  const tmp = join3(dirname3(path2), `.${process.pid}-${Math.random().toString(36).slice(2)}.tmp`);
...
async function fetchAllEntries(apiKey, path2, query) {
  const entries = [];
  let cursor;
  let pages = 0;
  do {
    ...
    if (cursor) q.pagination_cursor = cursor;
    const resp = await callLuma({ apiKey, method: "GET", path: path2, query: q });
    if (Array.isArray(resp)) return resp;
