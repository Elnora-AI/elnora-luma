 ...
 *
 * .env files are parsed with a strict 3-key allowlist; nothing outside the
 * config directory or the CLI's own folder is ever read. No credentials leave
 * the machine except to Luma's own API hosts (see src/client.ts) and — for the
 * optional STRIPE_API_KEY, used only by `luma stripe reconcile` — to
 * api.stripe.com (see src/stripe.ts; GET-only, use a restricted rk_ key).
 */
...
const ALLOWED_ENV_KEYS = new Set(["LUMA_API_KEY", "LUMA_AUTH_SESSION_KEY", "STRIPE_API_KEY"]);
